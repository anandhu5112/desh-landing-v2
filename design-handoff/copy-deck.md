# Desh — Website Copy

Every line of user-facing copy on the site, in the order a visitor encounters it.
Labels in brackets describe the role of each line, not the words themselves.

**Context for review:** Desh is an investing platform for NRIs (Non-Resident
Indians). It offers Indian mutual funds and US stocks, with a dedicated human
advisor. This is a single-page marketing site. The audience is Indians living
abroad — primarily US, UK, UAE, Singapore, and Australia — who want to invest
in India, in US markets, or both.

---

## Navigation

[Logo] Desh
[Link] Company
[Link] Community
[Button] Contact us

---

## Hero

[H1] Invest like a true global citizen

[Body] With roots in India and a life abroad, NRIs deserve a world of possibilities for their wealth.

[Button] Talk to an Advisor

*As the visitor scrolls, the above fades out and this replaces it:*

[H2] Anchor your roots.
     Expand your reach.

---

## Section 1 — Global investing (GIFT City outbound)

[H2] Hedge against the depreciating rupee

[Body] Own a slice of the world's biggest companies through USD funds regulated in GIFT City. No US brokerage account, no US tax filing.

---

## Section 2 — Advisors

[H2] Got questions about
     your money?

[Body] Whether you're starting your investment journey or managing a growing portfolio, receive personalised guidance whenever you need it.

[Button] Book a Free Consultation

*Text visible inside the sample portfolio graphic alongside this section:*

[Label] Total value
[Status] Monitoring
[Figure] ₹ 24,72,480
[Change] +12.84%   +₹5,09,776
[Timestamp] updated 2 mins ago

---

## Section 3 — Indian mutual funds

[H2] Grow wealth beyond FDs

[Body] Access professionally managed mutual funds that help NRIs participate in India's long term growth with confidence and convenience.

[Button] Talk to an Advisor

---

## Section 4 — SIP calculator

[H2] Wealth grows with time

[Tagline] Adjust the sliders and watch your wealth grow.

[Input label] Monthly Investment
[Input label] Select Investment Duration
[Duration options] 5 yrs · 10 yrs · 15 yrs · 20 yrs · 25 yrs · 30+ yrs
[Result label] Final Wealth

[Disclaimer] Illustrative projection at an assumed 12% annual return. Investments are subject to market risk; past performance does not predict future results.

---

## Section 5 — Portfolio CTA

[H2] Invest like a true
     global citizen

[Body] Get expert advice when you need it, or connect with fellow investors for ideas, updates, and learning.

[Button] Talk to an Advisor

---

## Section 6 — WhatsApp community card

[Heading] Join our exclusive NRI WhatsApp community.

[Body] Get guided, get invested, and build wealth back home from wherever you are in the world.

[Button] Join Our Community

---

## Section 7 — FAQ

[H2] Frequently Asked Questions

[Q] Who can invest through Desh?
[A] Desh supports NRIs and OCIs in most countries. We do not currently serve residents of the United States or Canada. Eligibility may also vary by country and product.

[Q] Do I need to be in India to start investing?
[A] No. You can complete onboarding remotely without visiting India, provided you have an NRE or NRO account.

[Q] What investment options do you offer?
[A] Indian mutual funds, and global markets through USD funds regulated in GIFT City, alongside curated portfolios built around your goals, time horizon, and risk appetite.

[Q] Will I get a dedicated advisor?
[A] Yes. Every investor is paired with a dedicated advisor you can reach whenever you need guidance, not a rotating support queue.

[Q] Can I start a SIP from overseas?
[A] Yes. You can set up and manage a SIP online from your NRE or NRO account, with contributions debited automatically each month.

[Q] Is my money held by Desh?
[A] No. Desh does not hold your money. Your investments are made through your own accounts.

[Closing tagline] A modern investing experience built for NRIs who want a clean, credible path into Indian mutual funds & global markets without getting buried in paperwork, confusion, or bad advice.

---

## Footer

[Copyright] © 2026 Desh
[Copyright] All rights reserved

---

## Contact modal — "Contact us" tab

[H2] Connect with us

[Tab] Contact us
[Tab] Join us

*Three stats rotate one at a time beside the form:*

[Stat] ₹1 Crore+ — Assets Guided
       Building wealth with a disciplined, long-term investment approach.

[Stat] 100+ — Happy NRI Investors
       Trusted by NRIs worldwide to invest in India's growth.

[Stat] Always — Human Support
       Real advisors, real conversations, whenever you need them.

*Form — visitor picks one reason:*

[Option] Start my investment journey
[Option] Explore Indian mutual funds
[Option] I want to Invest in US stocks
[Option] I need help in NRI documentation
[Option] Review my portfolio
[Option] Something else

[Placeholder] Enter your message here
[Button] Send message

---

## Contact modal — "Join us" tab

[H2] Join us

*Three stats rotate one at a time:*

[Stat] 999+ — Active NRI Investors
       Learn. Connect. Grow together.

[Stat] 100% — Personalized Guidance
       No generic recommendations. Every portfolio is tailored to your goals.

[Stat] 24–48 hrs — Response Time
       Quick assistance for investments, documentation, and portfolio reviews.

[Badge] +999
[Heading] Join our exclusive NRI WhatsApp community.
[Button] Join Our Community

---

*Not yet written: the browser tab title and search-result description are still
unedited scaffolding, and there is no privacy policy, terms, or regulatory
disclosure copy anywhere on the site.*
