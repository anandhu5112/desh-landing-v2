...
/*
 * Vendored third-party component, kept verbatim.
 * `set-state-in-effect` is disabled deliberately: the isMounted flag is the
 * SSR guard this component needs (it calls Math.random(), so sphere positions
 * must be computed client-side only), and refactoring vendored code would
 * risk breaking it on the next upstream paste.
 */
...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  maxRotationSpeed = 5,
  baseImageScale = 0.12,
  // Unused in the upstream source (renderImageNode hardcodes 1.2). Kept as-is
  // rather than "fixed", so this file stays identical to the vendored original.
  ...
  // `velocity` is intentionally a ref, not state: nothing renders from it, so
  // holding it in state only forced a re-render of all 60 nodes every frame.
  ...
  // Live mirror of `velocity` so the momentum loop can read it without taking a
  // state dependency (which previously re-created the callback every frame).
  ...
  const updateMomentum = useCallback(() => {
    ...
    const decayed = {
      x: prevVelocity.x * momentumDecay,
      y: prevVelocity.y * momentumDecay
    };

    // Snap to a hard zero once negligible. This must happen even when
    // autoRotate is on: otherwise the decay only ever approaches zero
    // asymptotically, so momentum is never considered "settled" and the
    // rotation keeps being nudged by an infinitesimal leftover value.
    const settled = Math.abs(decayed.x) < 0.01 && Math.abs(decayed.y) < 0.01;
    const nextVelocity = settled ? { x: 0, y: 0 } : decayed;

    // Only touch velocity state when the value actually changed.
    if (nextVelocity.x !== prevVelocity.x || nextVelocity.y !== prevVelocity.y) {
      velocityRef.current = nextVelocity;
    }

    // Nothing to animate: momentum has settled and auto-rotation is off.
    if (!autoRotate && nextVelocity.x === 0 && nextVelocity.y === 0) return;
    ...
    setRotation(prev => {
      ...
      // Add momentum-based rotation (current value, not a stale closure read)
      newY += clampRotationSpeed(nextVelocity.y);
  ...
  // Always points at the newest updateMomentum without being a dependency of
  ...
  useEffect(() => {
    ...
    // Depends only on isMounted, so the loop is scheduled once and runs
    // uninterrupted. Previously `updateMomentum` was a dependency, and since it
    // was re-created on every velocity change this effect tore the loop down
    // and re-scheduled it every single frame — the source of the judder.
    const animate = () => {
